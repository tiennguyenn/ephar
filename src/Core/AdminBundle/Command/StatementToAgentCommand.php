...
class StatementToAgentCommand extends ContainerAwareCommand
{
    /**
     * Send statement to agent
     * php app/console app:statement-to-agent [Y-m-d]
     * related to cronjob:
     * - agent:montly-statement:export-pdf [Y-m-d]
     * @author vinh.nguyen
     */
    protected function configure()
    {
        $this->setName('app:statement-to-agent')
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $workingDate = Utils::getWorkingDate($listPHDate, $psObj["agentStatementDate"]);

        if($dateTime->format('d') == $workingDate->format('d') || $day) {
            $output->writeln('============= Start ===========');
            $output->writeln($dateTime->format("Y-m-d H:i:s").': Statement To Agent');

            $result = $this->getContainer()->get('microservices.po')->sendStatementToAgent($day);
            if ($result) {
                $output->writeln("Total: ". count($result));
                foreach ($result as $k => $v) {
                    $output->writeln("Statement sent to agent ".$v . ".");
                }
            } else {
