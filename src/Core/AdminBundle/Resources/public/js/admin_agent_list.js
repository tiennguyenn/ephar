...
//            }, 200 );
    ...
    getData: function () {
        this.dataPost = {'search': this.tableData, 'length': this.tableLength, 'page': this.currentPage, 'status': this.agentStatus, 'sort': this.dataSort};
        $.ajax({
            type: "POST",
            url: this.ajaxUrl,
            data: this.dataPost,
    ...
    generateView: function (data)
    {
        ...
        var subPath = $("#sub-agent-path").val();
		var loginPath = $("#login-agent-path").val();
        for (var i = 0; i < arrayLength; i++)  {
            var curReactivatePath = reactivatePath.replace('id', data[i].id);
            var curPath = path.replace('id', data[i].hashId);
            var curSubPath = subPath.replace('id', data[i].hashId);
            ...
            if (DataTable.agentStatus != -1) {
                ...
                result += '<a href="' + curSubPath + '" class="btn btn-ahalf-circle text-uppercase green-seagreen btn-icon-right btn-xs">Sub Agents <i class="fa fa-user"></i></a>';
                ...
                result += '<a href="javascript:void(0)" data-agent-id="' + data[i].id + '" data-href="' + curReactivatePath + '" class="btn btn-ahalf-circle reactivate-agent text-uppercase green-seagreen btn-icon-right btn-xs">Re-activate</a>';
            }
            var txtLL = 'N/A';
