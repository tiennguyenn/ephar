var Form = {
    ...
    updateFixTotalValue(){
        ...
        $("#rd_fee_agent_local").val(jsCommon.currencyFormat(100 - $('#admin_agent_fee3rdLcMedicine').val()));
        $("#rd_fee_agent_oversea").val(jsCommon.currencyFormat(100 - $('#admin_agent_fee3rdOsMedicine').val()));
        $('#admin_agent_fee3rdLcMedicine').on('change', function () {
            if($(this).val().length ==  0 || $(this).val() > 100 ) {
                $(this).val(jsCommon.currencyFormat('0'));
            }
            $(" #rd_fee_agent_local").val(jsCommon.currencyFormat(100 - $(this).val()));
        });
        $('#rd_fee_agent_local').on('change', function () {
            if($(this).val().length ==  0 || $(this).val() > 100 ) {
                $(this).val(jsCommon.currencyFormat('0'));
            }
            $("#admin_agent_fee3rdLcMedicine").val(jsCommon.currencyFormat(100 - $(this).val()));
        });
        $('#admin_agent_fee3rdOsMedicine').on('change', function () {
            if($(this).val().length ==  0 || $(this).val() > 100 ) {
                $(this).val(jsCommon.currencyFormat('0'));
            }
            $("#rd_fee_agent_oversea").val(jsCommon.currencyFormat(100 - $(this).val()));
        });
        $('#rd_fee_agent_oversea').on('change', function () {
            if($(this).val().length ==  0 || $(this).val() > 100 ) {
                $(this).val(jsCommon.currencyFormat('0'));
            }
            $("#admin_agent_fee3rdOsMedicine").val(jsCommon.currencyFormat(100 - $(this).val()));
        });
    },
    initStatus: function () {
        var date = new Date();
        ...
        jsCommon.viewLogs('#view_logs_anchor_2', '#admin_view_logs_url');
        ...
        $("#admin_agent_site").on('change', function () {
            console.log($(this).val());
           if($(this).val() == 2){
               $("#third-party-agent-fee-area").removeClass("hidden");
           } else {
               $("#third-party-agent-fee-area").addClass("hidden");
           }
        });

        $('#admin_agent_check3rd').on('ifChecked', function (event) {
            $("#list-agent-third-party-fee").find('input').removeAttr("disabled");
            $("#list-agent-third-party-fee").removeClass("hidden");
        });
        $('#admin_agent_check3rd').on('ifUnchecked', function (event) {
            $("#list-agent-third-party-fee").find('input').attr("disabled",true);
            $("#list-agent-third-party-fee").addClass("hidden");
        });
            ...
            errorPlacement: function (error, element) {
                ...
                switch (element.attr("name")) {
                    ...
                    case 'admin_agent[fees][1][takeEffectOn]':
                        ...
                        break;
                    case 'admin_agent[date3rdLcMedicine]':
                    case 'admin_agent[date3rdOsMedicine]':
                    case 'admin_agent[date3rdLcPrescription]':
                    case 'admin_agent[date3rdOsPrescription]':
                    case 'admin_agent[date3rdLcConsult]':
                    case 'admin_agent[date3rdOsConsult]':
                        $e.parent().after(error);
                        break;
                    default :
