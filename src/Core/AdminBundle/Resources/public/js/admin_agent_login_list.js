var DataTable = $.extend(Base, {
    ...
    init: function () {
        this.suggestion(DataTable.ajaxUrl, {id: DataTable.agentId}, 'name');
        ...
        $('.filter-changing a').on('click', function () {
            ...
            DataTable.getData();
            DataTable.suggestion(DataTable.ajaxUrl, {id: DataTable.agentId}, 'name');
        });
    ...
    getData: function () {
        this.dataPost = {'search': this.tableData, 'length': this.tableLength, 'page': this.currentPage, 'status': this.agentStatus, 'sort': this.dataSort, 'id': this.agentId, 'role': 'agent', 'action': 'get_list'};
        $.ajax({
            type: "POST",
            url: this.ajaxUrl,
            data: this.dataPost,
    ...
    initEventTable: function () {
        $('.make-switch').on('switchChange.bootstrapSwitch', function () {
            ...
            if (_this.prop('checked', true)) {
                var dataPost = {'id': this.value, 'action': 'change_status', 'role': 'agent'};
                $.ajax({
                    type: "POST",
        ...
        $(".delete-agent-login").on('click', function (e) {
            e.preventDefault();
			DataTable.currentDeleteAgentLogin = $(this).attr('data-id');
			var email = $(this).attr('data-email');
			$("#modal-confirm-delete").find("p").html("Do you want to delete this agent login (" + email + ")?")
    ...
    generateView: function (data)
    {
        ...
        for (var i = 0; i < arrayLength; i++)  {
            ...
            if (DataTable.agentStatus != -1) {
                if (data[i].status) {
                    result += '<input type="checkbox" value="' + data[i].id + '" class="make-switch" checked  data-on-color="success" data-off-color="danger" data-on-text="' + "<i class='fa fa-check'></i>" + '" data-off-text="' + "<i class='fa fa-times'></i>" + '"  data-size="mini" ' + (!data[i].agentStatus ? 'disabled="" ' : '') + '>'
                            + '<div class="switch-text" >'
                            + '<span class="switch-text-on">Active</span>'
                            + '<span class="switch-text-off hide-item">Deactivated</span>';
                } else {
                    result += '<input type="checkbox" value="' + data[i].id + '" class="make-switch"  data-on-color="success" data-off-color="danger" data-on-text="' + "<i class='fa fa-check'></i>" + '" data-off-text="' + "<i class='fa fa-times'></i>" + '"  data-size="mini" ' + (!data[i].agentStatus ? 'disabled="" ' : '') + '>'
                            + '<div class="switch-text">'
            ...
            if (DataTable.agentStatus != -1) {
                ...
                result += '<a  data-id="' + data[i].id + '" data-toggle="modal" class="btn btn-ahalf-circle text-uppercase red btn-icon-right btn-xs btn-delete delete-agent-login" data-email="' + data[i].email + '" data-id="' + data[i].id + '">Delete</a>';
            }
            result += '</td>'
                    + '</tr>';
	...
	deleteLogin: function(btn) {
		...
		$(btn).html("Deleting...");
		var dataPost = {"id": DataTable.currentDeleteAgentLogin, "role": "agent", "action": "delete"};
		$.ajax({
			type: "POST",
			url: DataTable.ajaxUrl,
			data: dataPost,
