...
class MessageController extends BaseController
{
    /**
     * Messages compose
     * @Route("/messages/compose", name="doctor_message_compose")
     */
    public function composeAction(Request $request)
    {
        ...
        if(!empty($id)) {
            ...
            if($data == null)
                return $this->redirect($this->generateUrl('doctor_message'));
            $data['type'] = $type;
        } else if ($drugName) {
            $doctorName = $this->getUser()->getDisplayName();
            $parameters = array(
                'doctorName' => $doctorName,
                'drugName' => $drugName
            );
            $body = $this->renderView('DoctorBundle:rx:report-missing-medicine-body.html.twig', $parameters);
            $data = array();
            $data['type'] = '';
            $data['body'] = $body;
            $data['to'] = 'Ask the Pharmacist, Pharmacy Service';
            $data['subject'] = 'Request for Medication';
        } else if ($drugId) {
            $drugRepository = $em->getRepository('UtilBundle:Drug');
            $drug = $drugRepository->getDrugBy($drugId);
            $ingredients = $drugRepository->getDrugIngredients($drugId);

            $drugIngredients = array();
            foreach ($ingredients as $ingredient) {
                $drugIngredients[] = $ingredient->getName();
            }

            $doctorName = $this->getUser()->getDisplayName();
            $parameters = array(
                'doctorName' => ltrim($doctorName, 'Dr'),
                'drugName' => $drug[0]->getName(),
                'activeIngredients' => implode(', ', $drugIngredients),
                'manufacturer' => $drug[0]->getManufacturer()->getName(),
                'packingQuantity' => $drug[0]->getPackQuantity(),
                'packingType' => $drug[0]->getDosageFormRoute()->getDosageForm()->getName()
            );
            $body = $this->renderView('DoctorBundle:rx:report-out-of-stock-medicine-body.html.twig', $parameters);
            $data = array();
            $data['type'] = '';
            $data['body'] = $body;
            $data['to'] = 'Ask the Pharmacist, Pharmacy Service';
            $data['subject'] = 'Request for Re-stock';
        } else {
            $data = array();
        }
        $options = array(
            'attr' => array(
                ...
                'action'  => $this->generateUrl('doctor_message_send')
                ),
            'method' => 'POST'
        );
        ...
        return $this->render('DoctorBundle:message:compose.html.twig',[
    ...
    /**
     * Messages send
     * @Route("/messages/send", name="doctor_message_send")
     * @author vinh.nguyen
     */
    public function sendAction(Request $request)
    {
        ...
        if($request->isMethod('POST')) {
            ...
            switch ($postData['postType']) {
                case 'send':
                    $emailTarget = Common::getTargetEmail($em, $postData['to'], 'doctor');
                    // dump($sender); exit;
                ...
                case 'draft':
                    ...
                    $em->getRepository('UtilBundle:Message')->create($messageData, $attachments);
                    $this->get('session')->getFlashBag()->add('success', 'The message has been draft successfully');
                    return $this->redirect($this->generateUrl('doctor_message'));
                    break;
            }
        }
        return $this->redirect($this->generateUrl('doctor_message_compose'));
    }

    /**
     * Messages compose
     * @Route("/messages/view/{id}", name="doctor_message_view")
     * @author toan.le
     */
    public function viewAction(Request $request, $id)
    {
        ...
        return $this->render('DoctorBundle:message:view.html.twig',[
            'data' => $result,
    ...
    /**
     * Messages change
     * @Route("/messages/change", name="doctor_message_change")
     * @author vinh.nguyen
     */
    ...
    /**
     * Messages compose
     * @Route("/messages/upload", name="doctor_message_upload")
     */
    ...
    /**
     * download file
     * @Route("/messages/download", name="doctor_message_download")
     * @author vinh.nguyen
    **/
    ...
    /**
     * for suggestion
     * @Route("/messages/suggestion", name="doctor_message_suggestion")
     */
    public function suggestionAction(Request $request)
    {
        ...
        $results = array();
        $em = $this->getDoctrine()->getManager();
        $results = $em->getRepository('UtilBundle:Message')->suggestionSearch($term, 'doctor');
    ...
    /**
     * Get list messages
     * @Route("/messages/list", name="doctor_message_list")
     * @author vinh.nguyen
     */
    public function ajaxListAction()
    {
        ...
        $template = 'DoctorBundle:message:ajax_list.html.twig';
        $totalPages = isset($results['totalPages']) ? $results['totalPages'] : Constant::TOTAL_PAGE_DEFAULT;
        $pageUrl = $this->generateUrl('doctor_message_list', $params);
    ...
    /**
     * Messages
     * @Route("/messages/{type}", name="doctor_message")
     */
    public function indexAction($type = 'inbox')
    {
        return $this->render('DoctorBundle:message:index.html.twig',[
            'type' => $type
            ]);
    }
}
